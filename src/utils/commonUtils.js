export const checkQuestionId = (questionId, questionCount) => {
  ...
  if (
    parseInt(questionId) > parseInt(questionCount) ||
...
export const triggerConstant = async (contractObj, methodName, ...params) => {
  try {
    if (params.length > 0) {
      return await contractObj[methodName](...params).call({
        _isConstant: true,
      });
    } else {
    ...
    console.error(
      `triggerConstant contractObj=${contractObj}, methodName=${methodName}, params=${params}, err=${err}`
    );
    return "";
  }
};
